...
### CSRF Protection with Unobtrusive JavaScript (UJS)

With UJS we need to generate and pass the CSRF token within an ajax call. There are two steps required to do this.

1. Make sure the token is generated by adding this code to the application `<head>` section: `<%= csrf_meta_tags %>`.
2. Pass the token with the ajax call:
```
